...
typedef struct {
    ...
    XMeasure_con_uio_map maps[ MAX_UIO_MAPS ];
} XMeasure_con_uio_info;

/***************** Variable Definitions **************************************/
static XMeasure_con_uio_info uio_info;
...
static int uio_info_read_name(XMeasure_con_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMeasure_con_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMeasure_con_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMeasure_con_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMeasure_con_Initialize(XMeasure_con *InstancePtr, const char* InstanceName) {
	XMeasure_con_uio_info *InfoPtr = &uio_info;
